...
<ArticleHero 
  ...
  description="利用一阶微分形式不变性，把链式法则写成逐层微分的计算流程。属于 [微积分教学体系](/teaching/calculus) 中的导数计算专题。"
/>
...
**微分万能公式** 本质上是对一阶微分形式不变性的固定用法。莱布尼茨（Leibniz）的 $\mathrm{d}y$ 和 $\mathrm{d}x$ 符号之所以适合计算，是因为它们可以清楚记录变量之间的变化关系。
