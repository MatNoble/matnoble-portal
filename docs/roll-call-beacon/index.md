...
<strong>点名光标</strong>（`roll-call-beacon`）是一款专为高校教学场景设计的浏览器扩展，现已上架 Microsoft Edge 扩展市场。它与学校教务系统的考勤页面深度结合，旨在帮助教师在课堂上进行高效、随机的课堂互动与点名抽考。

该扩展遵循 <strong>“纯本地计算、零网络请求”</strong> 的设计原则，在保障学校教务敏感数据绝对安全的前提下，通过轻量化的交互设计提升课堂点名效率。
...
- <strong>黄金比例模式</strong>：以美学与随机平衡的黄金比例（约 61.8% 或 38.2%）快速筛选。
