...
% 提示 1：处理极端长宽比（数字 1）
% 数字 1 的宽高比极小，如果强行切分会导致网格重叠。
% 利用 w / h < 0.35 的先验知识，遇到极窄的边框直接判定为数字 1 并 continue 跳过后续测算。

% 提示 2：智能靶向采样边界设计（消除串扰与斜体偏差）
% 利用 size 函数获取 digit_img 的高 H 和宽 W。
% 提取横向线段（上、中、下）时，高度取对应位置，宽度只取极窄的中间地带（如 W 的 40% 到 60%），避开左右竖线造成的串扰。
% 提取纵向线段（左上、右上、左下、右下）时，高度避开横线，宽度向中心放宽到 40% 左右，以包容“斜体”数码管的倾斜偏移。

% 提示 3：提取特征区域并计算均值
% 根据提示 2 设计的行列索引，提取 7 个对应的图像切片。
% 使用 mean(region(:)) 计算各切片内的白色像素占比。
% 设定一个宽容的阈值（如 > 0.15）进行比较，得到该段是否亮起的逻辑值。
% 将 7 个逻辑值拼成一个特征码数组，例如 code = [1, 1, 1, 1, 1, 1, 0]。

% 提示 4：特征码真值表匹配
% 使用 isequal(code, [1,1,1,1,1,1,0]) 函数将其与标准的 0-9 特征码真值表进行比对。
% 使用 if-elseif 结构输出最终识别的十进制数字。
```
