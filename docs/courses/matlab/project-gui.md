...
### 5.3 算力引擎上机测试：从理想解到数值灾难

在您完成了上述核心算力代码后，可以直接在 GUI 中尝试以下两组极具代表性的输入，来亲自验证“数值分析”中的边缘效应。因为我们底层使用的是 `str2num` 解析器，您可以直接在输入框中注入 MATLAB 内置函数：

#### 测试 1：常规良性矩阵（体验完美的浮点精度）
这是一个条件数极好的普通方程组，存在完美的精确解。
*   **矩阵 A 输入区**：`[4 3; 6 3]`
*   **矩阵 B 输入区**：`[10; 12]`

👉 **现象**：目标向量解为 `[1; 2]`，而计算残差范数通常是 `0` 或者极小的 `1.7e-15`（典型的计算机双精度浮点数截断下限），这代表算法在稳定状态下运行极其精确。

#### 测试 2：臭名昭著的“希尔伯特矩阵” (病态矩阵爆炸)
在数值分析领域，**希尔伯特矩阵 (Hilbert Matrix)** 常被用来测试算法的数值稳定性。当维度达到 12 阶时，它的条件数会飙升到 $10^{16}$ 级别，这正好击穿了计算机 64 位双精度浮点数的极限。

*   **矩阵 A 输入区**：`hilb(12)`
*   **矩阵 B 输入区**：`ones(12, 1)`

👉 **现象**：
1. 计算出来的 $x$ 向量会出现 `1.852e+08` 等极其离谱的天文数字，完全背离真实精确解。
2. 最底部的**残差范数**不再是极小值，而是膨胀到了 $10^{-9}$ 到 $10^{-4}$ 级别甚至更大。在双精度浮点数的世界里，这相当于误差被放大了整整一千万倍！
3. 此时如果查看背后的 MATLAB 命令行终端，系统很可能已经亮起了黄灯警告：`警告: 矩阵接近奇异值，或缩放错误。结果可能不准确。`

这就是数值计算中最震撼的“失真现象”，深刻说明了：在计算机工程中，就算理论数学公式绝对正确，如果输入数据“病态”，算出的结果也可能毫无意义。
