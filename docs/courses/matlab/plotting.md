...
*   **智能绘图 `fplot`**：
    将刚刚定义的句柄 `f` 传给 `fplot`，并指定绘图区间 `[xmin, xmax]` 即可。为了直观感受它的强大，我们可以使用 `subplot` 将传统的 `plot`（手动取 30 个点）与 `fplot` 放在一起对比：
    ```matlab
    % 左图：传统 plot 绘图（点数不够会导致剧烈震荡处变成折线甚至失真）
    subplot(1, 2, 1);
    x = linspace(0.01, 0.1, 30);  % 仅取 30 个点
    plot(x, sin(1./x), 'r-', 'LineWidth', 1.5);
    title('plot (手动取 30 个点)');
    
    % 右图：智能 fplot 绘图（自适应算法会在震荡处密集撒点，保持绝对平滑）
    subplot(1, 2, 2);
    f = @(x) sin(1./x);
    fplot(f, [0.01, 0.1], 'b-', 'LineWidth', 1.5);
    title('fplot (自适应撒点)');
    ```
