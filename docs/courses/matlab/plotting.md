...
*   **智能绘图 `fplot`**：
    将刚刚定义的句柄 `f` 传给 `fplot`，并指定绘图区间 `[xmin, xmax]` 即可。为了直观感受它的强大，我们可以使用 `subplot` 将传统的 `plot`（手动取 50 个点）与 `fplot` 放在一起对比：
    ```matlab
    % 左图：传统 plot 绘图（点数不够导致剧烈震荡处失真）
    subplot(1, 2, 1);
    x = linspace(0.01, 0.1, 50);  
    plot(x, sin(1./x), 'r-o');
    title('plot (手动取 50 个点)');
    
    % 右图：智能 fplot 绘图（自适应算法会在震荡剧烈处密集撒点）
    subplot(1, 2, 2);
    f = @(x) sin(1./x);
    fplot(f, [0.01, 0.1], 'b-*');
    title('fplot (自适应撒点)');
