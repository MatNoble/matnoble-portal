...
在开始编写复杂的程序前，必须掌握 MATLAB 的界面指令、变量与数据类型、矩阵表示及切片索引，以及核心的矩阵与元素级运算法则。
...
除了使用具体的行号和列号，MATLAB 还支持一种非常强大且直观的提取方式——**逻辑索引**。
当你将关系运算符（如 `>`、`<`、`==`）直接应用于矩阵时，会生成一个由 0 和 1 组成的同等大小的逻辑矩阵。将这个逻辑矩阵作为索引传入原矩阵，就能直接提取满足条件的所有元素（对于多维矩阵，提取结果会自动展平为一个列向量；对于行向量则保持不变）。
...
```
相比于官方文档中利用 `mod(A,2)` 判断奇偶后再强转为 `logical` 的数学技巧，**直接利用关系运算符（`>=`、`<`、`==` 等）配合 `logical()` 进行数据过滤**不仅更直观，也是日常数据分析中最常用的编程思维。
...
本实践项目仅需使用命令行或一个简单的脚本文件，旨在巩固变量操作、矩阵切片提取以及逻辑索引等核心知识点。
...
通过这个简单的小练习，你可以非常直观地感受到 MATLAB 中矩阵切片和逻辑索引带来的高效与便捷！
